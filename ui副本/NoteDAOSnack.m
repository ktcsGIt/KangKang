...
@implementation NoteDAOSnack

static NoteDAOSnack *sharedManager = nil;
...
+ (NoteDAOSnack*)sharedManager
{
    ...
    dispatch_once(&once, ^{
        ...
        [sharedManager createEditableCopyOfDatabaseIfNeededForSnack];
    });
...
- (void)createEditableCopyOfDatabaseIfNeededForSnack {
    ...
    NSString *writableDBPath = [self applicationDocumentsDirectoryFileForSnack];
    
    BOOL dbexits = [fileManager fileExistsAtPath:writableDBPath];
    if (!dbexits) {
        NSString *defaultDBPath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"snackList.plist"];
...
{
    NSString *path = [self applicationDocumentsDirectoryFileForSnack];
...
-(NSMutableArray*) findAll
{
    ...
    for (NSDictionary* dict in array) {
        ...
        NoteSnack* note = [[NoteSnack alloc] init];
